...
# Create folder
if not os.path.exists('./output2/satellite-image/post-processed-images'):
  os.makedirs('./output2/satellite-image/post-processed-images')
if not os.path.exists('./output2/shoreline/geojson'):
  os.makedirs('./output2/shoreline/geojson')
if not os.path.exists('./output2/shoreline/figure/shorelines'):
  os.makedirs('./output2/shoreline/figure/shorelines')
...
Image = rasterio.open('./output2/satellite-image/pre-processed-images/image_snrgb_10m_'+file_name+'.tif')
if Image.read().any() == 0:
...
output = os.path.join('./output2/satellite-image/post-processed-images/image_snrgb_2m_'+file_name+'.tif')
with rasterio.open(output, "w",tiled=True, compress='lzw', **out_meta) as dest:
    dest.write(rescale_image.astype(np.float32))
    
# rescale_image = rasterio.open('./output2/satellite-image/post-processed-images/image_snrgb_2m_'+file_name+'.tif')
...
outfp = './output2/shoreline/geojson/shoreline_'+file_name+'.json'
...
plt.tight_layout()
plt.savefig('./output2/shoreline/figure/shorelines/shoreline_'+file_name+'.png', dpi=300)
